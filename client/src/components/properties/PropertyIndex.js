      ...
      const getData = async () => {
        try {
          const { data } = await axios.get(
            `https://api.zoopla.co.uk/api/v1/property_listings.json?area=london&listing_status=sale&maximum_price=200000&page_size=100&api_key=${zooplaKey}`
          )
          setListings(data)
        } catch (err) {
          console.log(err)
